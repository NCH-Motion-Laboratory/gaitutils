...
for filepath_ in enffiles:
    ...
    if TRIALS_RANGE[0] <= int(trialn) <= TRIALS_RANGE[1]:
        ...
        if (trange[1] - trange[0]) < MIN_TRIAL_DURATION:
            ...
            # check gaps
            ctr = nexus.center_frame(vicon)
            gaps_found = False
            for marker in PIG_MARKERS:
                gaps = nexus.gaps(vicon, marker)
                # check for gaps nearby the center frame
                if gaps.size > 0:
                    if np.where(abs(gaps - ctr) < GAPS_MIN_DIST)[0].size > GAPS_MAX:
                        print('problem gaps:', gaps,
                              'marker:', marker, 'center:', ctr)
                        gaps_found = True
                        break
        if gaps_found:
...
print('\n2nd pass - processing %d trials' % len(sel_trials))
